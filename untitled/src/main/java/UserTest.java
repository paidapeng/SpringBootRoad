...
//    }
    ...
    public void writeExcel1HW(List<ExcelModelHW> list,String path) throws Exception {
        ExcelModelHW excelModel = new ExcelModelHW();
        // 文件输出位置
    ...
    public void writeExcel1ZTH(List<ExcelModelZTH> list,String path) throws Exception {
        ExcelModelZTH excelModel = new ExcelModelZTH();
        // 文件输出位置
        OutputStream out = new FileOutputStream(path);
...
//            ReadFromFile.showAvailableBytes(in);
     ...
     * @param fileName 文件名
     */
    public List readFileByLinesZTH(String fileName) {
        ...
        try {
            ...
            while ((tempString = reader.readLine()) != null) {
               ExcelModelZTH excelModel = new ExcelModelZTH();
                ...
                System.out.println("line " + line + ": " + fileLine.length+"   "+tempString);
                excelModel.setValue1(fileLine[0]);
                excelModel.setValue2(fileLine[1]);
                excelModel.setValue3(fileLine[2]);
                excelModel.setValue4(fileLine[3]);
                excelModel.setValue5(fileLine[4]);
                excelModel.setValue6(fileLine[5]);
                excelModel.setValue7(fileLine[6]);
                excelModel.setValue8(fileLine[7]);
                excelModel.setValue9(fileLine[8]);
                excelModel.setValue10(fileLine[9]);
                if (fileLine.length>=11){
                    excelModel.setValue11(fileLine[10]);

                }
                if (fileLine.length>=12){
                    excelModel.setValue12(fileLine[11]);
                }
                if (fileLine.length>=13){
                    excelModel.setValue13(fileLine[12]);
                }
                list.add(excelModel);
                // 显示行号  4294967295
//                System.out.println("line " + line + ": " + tempString);
     ...
     * @param fileName 文件名
     */
    public List readFileByLinesHW(String fileName) {
        List<Object> list = new ArrayList<>();
        File file = new File(fileName);
        BufferedReader reader = null;
        try {
