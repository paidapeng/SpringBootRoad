...
//        test.readFileByLines("F:/linshiwenjian.txt");
    ...
    public void writeExcel1(List<ExcelModel> list) throws Exception {
        ...
        // 文件输出位置
        OutputStream out = new FileOutputStream("F:/test.xlsx");
    ...
    public List readFileByLines(String fileName) {
        ...
        try {
            ...
            while ((tempString = reader.readLine()) != null) {
                ...
                System.out.println("line " + line + ": " + fileLine.length);
                excelModel.value1(fileLine[1]);
                excelModel.value2(fileLine[2]);
                excelModel.value3(fileLine[3]);
                excelModel.value4(fileLine[4]);
                excelModel.value5(fileLine[5]);
                excelModel.value6(fileLine[6]);
                excelModel.value7(fileLine[7]);
                excelModel.value8(fileLine[8]);
                excelModel.value9(fileLine[9]);
                excelModel.value10(fileLine[10]);
                if (fileLine.length>=11){
                        excelModel.value11(fileLine[11]);
                }
                if (file.length()>=12){
                    excelModel.value12(fileLine[12]);
                }
                list.add(excelModel);
                // 显示行号  4294967295
